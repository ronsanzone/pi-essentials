...
export default function ketchWebTools(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    label: "Web Search",
    description:
      "Search the web using ketch. Internally tries Brave first and falls back to DuckDuckGo if Brave fails or is rate-limited. Returns JSON-backed compact search results. Output is truncated to 50KB/2000 lines.",
    promptSnippet: "Search the web for current external information using ketch with Brave-to-DuckDuckGo fallback",
    promptGuidelines: [
      "Use web_search for external/current information discovery instead of calling ketch through bash.",
    ...
    parameters: Type.Object({
      ...
      maxChars: Type.Optional(
        Type.Number({ description: "Maximum markdown characters per scraped search result when scrape=true. Defaults to ketch behavior." }),
      ),
    }),
  ...
  pi.registerTool({
    ...
    label: "Web Scrape",
    description:
      "Fetch one or more URLs with ketch and return clean markdown. Handles JS-rendered pages when ketch browser support is configured. Defaults to maxChars=30000 per ketch invocation. Output is truncated to 50KB/2000 lines.",
    promptSnippet: "Scrape URLs to clean markdown using ketch",
    promptGuidelines: [
      "Use web_scrape to fetch full content only from URLs that look relevant and authoritative.",
      "Prefer web_scrape over calling ketch scrape through bash.",
    ],
    parameters: Type.Object({
